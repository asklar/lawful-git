...
lawful-git is a single-binary Go program that acts as a transparent drop-in replacement for `git`. It intercepts git invocations, enforces per-repo safety policies declared in `.lawful-git.json`, then `exec`s the real git binary. If no config is found, it passes through silently.
...
3. `loadConfig()` — loads global config (`~/.lawful-git.json` or `$LAWFUL_GIT_GLOBAL_CONFIG`), then repo config (`.lawful-git.json`), merges them (arrays union, booleans OR, maps merge with repo winning on conflict), and validates the result. Returns nil (passthrough) if no config exists at all.
...
- **Rule types** in `.lawful-git.json` map directly to code sections in `applyRules()`: `blocked`, `require`, `scoped_paths`, `worktree_only_branches`, `check_dirty_on_checkout`, `require_upstream_before_bare_push`, `protected_branches`. When adding a new rule type, add a new struct, a new field on `Config`, and a new section in `applyRules()`.
...
- **Fail-closed on config errors**: if `.lawful-git.json` exists but is malformed, lawful-git exits with an error rather than silently disabling rules.
- The repo's `examples/lawful-git.json` is a sample config — it restricts operations to the `my-project/` path prefix and blocks destructive commands.
